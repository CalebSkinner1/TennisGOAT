...
matches <- read_csv(here("data/gs_matches.csv"), show_col_types = FALSE)
players <- read_csv(here("data/gs_players.csv"), show_col_types = FALSE)
...
active <- players %>%
  ...
  ungroup() %>%
  filter(last_tournament > ymd(20260101)) %>%
  rename(
    Country = ioc,
    Player = name) %>%
