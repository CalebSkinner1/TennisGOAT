...
Rankings, timeline, and frequent matchups at Wimbledon. Wimbledon is a grass court tournament held in June and July near London, England.
...
#| fig.cap: Top 25 Players at Wimbledon
...
p <- players %>%
  group_by(name) %>%
  filter(tourney_name == "Wimbledon", name %in% top25) %>%
  mutate(
...
players %>%
  filter(score != "W/O", tourney_name == "Wimbledon") %>%
